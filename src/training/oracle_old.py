...
def log_oracle_training_header(args, config, num_classes, val_loader, train_loader):
    """Log Oracle training phase header and information."""
    print("================================================================================")
    print("🏋️  TRAINING PHASE")
    print("================================================================================")
    print("")
    ...
    print(f"   Classes          : {num_classes}")
    print(f"   Val Samples      : {len(val_loader.dataset) if val_loader else 0}")
    print(f"   Total Samples    : {len(train_loader.dataset) + (len(val_loader.dataset) if val_loader else 0)}")
    print("")
...
def train_model_oracle(config, args):
    ...
    from ..utils.logging import ICICLELogger
    from ..main_utils import setup_model_and_data, evaluate_epoch, evaluate_oracle_per_checkpoint
    ...
    # Import setup function (avoiding circular import)
    # model, train_loader, val_loader, test_loader = setup_model_and_data(
    #     config, args, mode='oracle'
    # )
    
    # For now, use a simpler approach to avoid circular imports
    # This will be refactored further
    logger.info("Oracle training function called, but implementation needs to be updated for new structure")
    return None  # Placeholder
    
    # Calculate and log per-class distribution
    oracle_class_distribution = calculate_oracle_class_distribution(
        train_data, test_data, val_loader
    )
    
    # Log training header
    log_oracle_training_header(args, config, num_classes, val_loader, train_loader)
    ...
    for epoch in range(hyperparams['num_epochs']):
        ...
        log_epoch_results(epoch, "TRAIN", train_loss, train_acc, train_acc)
        ...
        if args.train_val and val_loader and len(val_loader) > 0:
            ...
            log_epoch_results(epoch, " VAL ", val_loss, val_acc, val_bal_acc)
        
        # Run testing if requested
        if args.train_test:
            from ..main_utils import evaluate_oracle_per_checkpoint
            test_loss, test_acc, test_bal_acc, test_samples = evaluate_oracle_per_checkpoint(
                model, config, criterion, device
            )
            log_epoch_results(epoch, "TEST*", test_loss, test_acc, test_bal_acc)
