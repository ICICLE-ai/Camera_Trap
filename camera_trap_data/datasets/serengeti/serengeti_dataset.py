...
class SerengetiDataset:
    def __init__(self, dataset_config):
        """
        Serengeti-specific dataset class.

        Args:
            dataset_config (dict): Configuration for the Serengeti dataset.
        """
    ...
    def load_metadata(self):
        """
        Load and organize Serengeti-specific metadata based on the configuration.
        ...
        metadata_path = os.path.join(current_dir, "metadata", "serengeti_metadata.json")
        bbox_path = os.path.join(current_dir, "metadata", "serengeti_bbox.json")
        ...
        serengeti_config = self.dataset_config
        ...
        all_camera = serengeti_config.get("all_camera", False)
        cameras = serengeti_config.get("cameras", [])
    ...
    def summarize_data(self, organized_metadata):
        """
        Summarize the retrieved data for the Serengeti dataset.
    ...
    def _get_camera_data(self, dataset_name, cameras=None):
        """
        Retrieve data entries for cameras in Serengeti.
    ...
    def _format_location(self, location):
        """
        Format the location to add Serengeti at the beginning.

        Args:
            location (str): Original location string (e.g., "CA-30").

        Returns:
            str: Formatted location string (e.g., "serengeti_CA-30").
        """
