...
class ChannelIslandDataset:
    def __init__(self, dataset_config):
        """
        channel_island-specific dataset class.

        Args:
            dataset_config (dict): Configuration for the channel_island dataset.
        """
        self.dataset_config = dataset_config
        self.ds = None  # COCOCT object to store dataset
        self.image_root = "images"  # Root directory for images
        self.bbox_data = None  # To store loaded bbox data
        self.dataset_name = None

    def load_metadata(self):
        """
        Load and organize channel_island-specific metadata based on the configuration.
        ...
        self.dataset_name = os.path.basename(current_dir)  # 'channel_island'
        self.image_root = os.path.join(current_dir, "images")

        # Construct absolute paths for metadata and bbox files
        metadata_path = os.path.join(current_dir, "metadata", f"{self.dataset_name}_metadata.json")
        bbox_path = os.path.join(current_dir, "metadata", f"{self.dataset_name}_bbox.json")
        ...
        channel_island_config = self.dataset_config
        ...
        # if all_datasets:
            # selected_datasets = [key for key in safari_config.keys()
        ...
        all_camera = channel_island_config.get("all_camera", False)
        cameras = channel_island_config.get("cameras", [])
    ...
    def _get_bbox_data(self, organized_metadata):
        ...
        bbox_lookup = {entry["file"]: entry["detections"] for entry in self.bbox_data["images"] if "detections" in entry}

        # Iterate through the organized metadata and fill bbox data
        for dataset_name, dataset_metadata in organized_metadata.items():
            for entry in dataset_metadata["data"]:
                image_file = entry["image_path"].split("images/")[-1]  # Use image_id to match with bbox file
    ...
    def _get_camera_data(self, dataset_name, cameras=None):
        """
        Retrieve data entries for cameras in channel_island.
    ...
    def _format_location(self, location):
        """
        Format the location to add channel_island at the beginning.

        Args:
            location (str): Original location string (e.g., "CA-30").

        Returns:
            str: Formatted location string (e.g., "channel_island_CA-30").
        """
        # parts = location.split("_")
        # if len(parts) >= 2:
            # return f"{parts[0]}_{parts[-1]}"  # Keep only the first and last parts
        
    ds = getattr(self, "dataset_name", "channel_island")
    return f"{ds}_{location}"
