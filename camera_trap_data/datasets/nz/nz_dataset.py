...
class NzDataset:
    ...
    def load_metadata(self):
        ...
        self.image_root = os.path.join(current_dir, "images/nz-trailcams")

        # Construct absolute paths for metadata and bbox files
        metadata_path = os.path.join(current_dir, "metadata", "nz_metadata.json")
        bbox_path = os.path.join(current_dir, "metadata", "nz_bbox.json")
        ...
        all_camera = nz_config.get("all_camera", False)
        cameras = nz_config.get("cameras", [])
    ...
    def _get_bbox_data(self, organized_metadata):
        ...
        for dataset_name, dataset_metadata in organized_metadata.items():
            for entry in dataset_metadata["data"]:
                image_file = entry["image_path"].split("images/nz-trailcams/")[-1]  # Use image_id to match with bbox file
    ...
    def _format_location(self, location):
        ...
        Returns:
            str: Formatted location string (e.g., "nz_CA-30").
        """
