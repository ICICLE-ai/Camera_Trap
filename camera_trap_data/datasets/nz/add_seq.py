...
path = "/fs/ess/PAS2099/Vidhi/camera_trap_data/datasets/nz/metadata/trail_camera_images_of_new_zealand_animals_1.00.json"
...
for i, location in enumerate(locations):
    bins[i % 100].append(location)
node_locations = bins[node_id]
...
for image in tqdm(metadata["images"], desc = f"Node {node_id} processing images"):
    if image["location"] in node_locations:
        processed_count += 1
...
for location in node_locations:
    ...
    if all("datetime" in image and image["datetime"] is not None for image in filtered_images):
        ...
        for image_1, image_2 in tqdm(combinations(sorted_images, 2), desc = f"Node {node_id} assigning seq_ids for location {location}"):
...
output_path = f"/fs/ess/PAS2099/Vidhi/camera_trap_data/datasets/nz/metadata/trail_camera_images_of_new_zealand_animals_1.00_with_seq_{node_id}.json"
