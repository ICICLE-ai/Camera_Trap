...
class SafariDataset:
    def __init__(self, dataset_config):
        """
        Safari-specific dataset class.

        Args:
            dataset_config (dict): Configuration for the Safari dataset.
        """
    ...
    def load_metadata(self):
        """
        Load and organize Safari-specific metadata based on the configuration.
        ...
        metadata_path = os.path.join(current_dir, "metadata", "safari_metadata.json")
        bbox_path = os.path.join(current_dir, "metadata", "safari_bbox.json")
        ...
        safari_config = self.dataset_config
        ...
        for dataset_name in tqdm(selected_datasets, desc="Processing datasets", leave=False):
            ...
            all_camera = dataset_config.get("all_camera", False)
    ...
    def summarize_data(self, organized_metadata):
        """
        Summarize the retrieved data for Safari datasets.

        Args:
            organized_metadata (dict): The organized metadata structure.

        Returns:
            dict: A summary of datasets and their associated cameras.
        """
        summary = {}
    ...
    def _get_bbox_data(self, organized_metadata):
        ...
        for dataset_name, dataset_metadata in organized_metadata.items():
            for entry in dataset_metadata["data"]:
                image_file = entry["image_id"]  # Use image_id to match with bbox file
    ...
    def _get_camera_data(self, dataset_name, cameras=None):
        """
        Retrieve data entries for cameras in a specific dataset.
        ...
        data = [
            {
                ...
                "datetime": self.ds.imgs[img_id].get("datetime", None),
                "seq": {
            ...
            if self._get_dataset_name(self.ds.imgs[img_id]["location"]) == dataset_name  # Match dataset name
            and (cameras is None or self._format_location(self.ds.imgs[img_id]["location"]) in cameras)  # Match cameras
        ]

        return {
            dataset_name: {
                "dataset_name": f"safari_{dataset_name}",
    ...
    def _format_location(self, location):
        """
        Format the location to remove the middle part.

        Args:
            location (str): Original location string (e.g., "APN_S2_13U").

        Returns:
            str: Formatted location string (e.g., "APN_13U").
        """
