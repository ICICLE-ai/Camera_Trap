...
class CaltechDataset:
    def __init__(self, dataset_config):
        """
        caltech-specific dataset class.

        Args:
            dataset_config (dict): Configuration for the caltech dataset.
        """
        self.dataset_config = dataset_config
        self.ds = None  # COCOCT object to store dataset
        self.image_root = "/fs/scratch/PAS2099/camera-trap/caltech"  # Root directory for images
        self.bbox_data = None  # To store loaded bbox data

    def load_metadata(self):
        """
        Load and organize caltech-specific metadata based on the configuration.
        ...
        metadata_path = os.path.join(current_dir, "metadata", "caltech_metadata.json")
        bbox_path = os.path.join(current_dir, "metadata", "caltech_bbox.json")
        ...
        caltech_config = self.dataset_config
        ...
        all_camera = caltech_config.get("all_camera", False)
        cameras = caltech_config.get("cameras", [])
    ...
    def _get_bbox_data(self, organized_metadata):
        ...
        for dataset_name, dataset_metadata in organized_metadata.items():
            for entry in dataset_metadata["data"]:
                image_file = entry["image_path"].split("caltech/")[-1]  # Use image_id to match with bbox file
    ...
    def _get_camera_data(self, dataset_name, cameras=None):
        """
        Retrieve data entries for cameras in caltech.
    ...
    def _format_location(self, location):
        """
        Format the location to add caltech at the beginning.

        Args:
            location (str): Original location string (e.g., "CA-30").

        Returns:
            str: Formatted location string (e.g., "caltech_CA-30").
        """
        # parts = location.split("_")
        # if len(parts) >= 2:
            # return f"{parts[0]}_{parts[-1]}"  # Keep only the first and last parts
        
        dataset_name = "caltech"  # Hardcoded for caltech
