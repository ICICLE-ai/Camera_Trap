...
class SeattleDataset:
    def __init__(self, dataset_config):
        """
        seattle-specific dataset class.

        Args:
            dataset_config (dict): Configuration for the seattle dataset.
        """
    ...
    def load_metadata(self):
        """
        Load and organize seattle-specific metadata based on the configuration.
        ...
        metadata_path = os.path.join(current_dir, "metadata", "seattle_metadata.json")
        bbox_path = os.path.join(current_dir, "metadata", "seattle_bbox.json")
        ...
        seattle_config = self.dataset_config
        ...
        all_camera = seattle_config.get("all_camera", False)
        cameras = seattle_config.get("cameras", [])
    ...
    def _get_bbox_data(self, organized_metadata):
        ...
        for dataset_name, dataset_metadata in organized_metadata.items():
            for entry in dataset_metadata["data"]:
                ...
                image_file = f'camera_trap_images/{image_file}'  # Adjust path to match bbox file structure
    ...
    def _get_camera_data(self, dataset_name, cameras=None):
        """
        Retrieve data entries for cameras in seattle.
    ...
    def _format_location(self, location):
        """
        Format the location to add seattle at the beginning.

        Args:
            location (str): Original location string (e.g., "CA-30").

        Returns:
            str: Formatted location string (e.g., "seattle_CA-30").
        """
        # parts = location.split("_")
        # if len(parts) >= 2:
            # return f"{parts[0]}_{parts[-1]}"  # Keep only the first and last parts
        
        dataset_name = "seattle"  # Hardcoded for seattle
