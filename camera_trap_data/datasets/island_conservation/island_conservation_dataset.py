...
class IslandConservationDataset:
    def __init__(self, dataset_config):
        """
        Island Conservation-specific dataset class.

        Args:
            dataset_config (dict): Configuration for the Island Conservation dataset.
        """
    ...
    def load_metadata(self):
        ...
        metadata_path = os.path.join(current_dir, "metadata", "island_conservation_metadata.json")
        bbox_path = os.path.join(current_dir, "metadata", "island_conservation_bbox.json")
        ...
        island_conservation_config = self.dataset_config
        ...
        all_camera = island_conservation_config.get("all_camera", False)
        cameras = island_conservation_config.get("cameras", [])
    ...
    def _get_camera_data(self, dataset_name, cameras=None):
        """
        Retrieve data entries for cameras in Island Conservation.
    ...
    def _format_location(self, location):
        """
        Format the location to add Island Conservation at the beginning.

        Args:
            location (str): Original location string (e.g., "CA-30").

        Returns:
            str: Formatted location string (e.g., "island_conservation_CA-30").
        """
        # parts = location.split("_")
        # if len(parts) >= 2:
            # return f"{parts[0]}_{parts[-1]}"  # Keep only the first and last parts
        
        dataset_name = "island_conservation"  # Hardcoded for Island Conservation
