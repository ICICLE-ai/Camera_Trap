...
class MissouriDataset:
    def __init__(self, dataset_config):
        """
        Missouri-specific dataset class.

        Args:
            dataset_config (dict): Configuration for the missouri dataset.
        """
    ...
    def load_metadata(self):
        """
        Load and organize missouri-specific metadata based on the configuration.
        ...
        metadata_path = os.path.join(current_dir, "metadata", "missouri_metadata.json")
        bbox_path = os.path.join(current_dir, "metadata", "missouri_bbox.json")
        ...
        missouri_config = self.dataset_config
        ...
        dataset_name = "missouri"

        all_camera = missouri_config.get("all_camera", False)
        cameras = missouri_config.get("cameras", [])
    ...
    def _get_camera_data(self, dataset_name, cameras=None):
        """
        Retrieve data entries for cameras in missouri.
    ...
    def _format_location(self, location):
        """
        Format the location to add missouri at the beginning.

        Args:
            location (str): Original location string (e.g., "CA-30").

        Returns:
            str: Formatted location string (e.g., "missouri_CA-30").
        """
        # parts = location.split("_")
        # if len(parts) >= 2:
            # return f"{parts[0]}_{parts[-1]}"  # Keep only the first and last parts
        
        dataset_name = "missouri"  # Hardcoded for missouri
