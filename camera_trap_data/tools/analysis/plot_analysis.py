...
class PlotAnalysis:
    def __init__(self, config, dataset, analysis_path, checkpoint_days=30):
        """
    ...
    def _plot_ckp_piecharts(self):
        ...
        for i, (dataset_name, cameras) in enumerate(self.dataset.metadata.items()):
            for camera_name, camera_data in tqdm(cameras.items(), desc=f"({i} / {total_len}) Processing {dataset_name}'s cameras", leave=False):
                ...
                # Prepare paths for saving plots (day-specific)
                camera_path = os.path.join(self.analysis_path, dataset_name, camera_name, str(self.checkpoint_days), "plots", "ckp_piechart")
    ...
    def _plot_count_histograms(self):
        ...
        for i, (dataset_name, cameras) in enumerate(self.dataset.metadata.items()):
            for camera_name, camera_data in tqdm(cameras.items(), desc=f"({i} / {total_len}) Processing {dataset_name}'s cameras", leave=False):
                ...
                # Prepare paths for saving plots (day-specific)
                camera_path = os.path.join(self.analysis_path, dataset_name, camera_name, str(self.checkpoint_days), "plots", "histograms")
